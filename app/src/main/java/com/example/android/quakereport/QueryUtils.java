...
public final class QueryUtils {
    ...
    /**
     * Return a list of {@link Eartquake} objects that has been built up from
     * parsing a JSON response.
     */
    public static List<Eartquake> extractEarthquakes(String inputJson) {

        // Create an empty ArrayList that we can start adding earthquakes to
        List<Eartquake> earthquakes = new ArrayList<>();
        ...
        try {
            ...
            JSONObject jsonRoot = new JSONObject(inputJson);
            JSONArray jsonEarthquakes = jsonRoot.optJSONArray("features");
            for (int i = 0; i < jsonEarthquakes.length(); i++){
                JSONObject row = jsonEarthquakes.getJSONObject(i);
                row = row.getJSONObject("properties");
                Eartquake earthquake = new Eartquake(row.optDouble("mag"),row.optString("place"),row.optLong("time"),row.optString("url"));
    ...
    public static List<Eartquake> fetchEarthquakeData(String requestUrl, Context context) {
        ...
        List<Eartquake> earthquakes = extractEarthquakes(jsonResponse);
